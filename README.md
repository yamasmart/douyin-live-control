...
- **AI 扩写**：给任一条快捷评论一键生成、或在草稿上扩写一条 ≤50 字、突出卖点和福利、引导下单的口播式短评论（据本场商品名）。走 OpenAI 兼容接口，密钥由用户在「设置」里**自填、仅存本机**（BYO-key），不内置、不上传。
...
src/main/
  ...
  llm.ts             AI 扩写：OpenAI 兼容 /chat/completions 调用 + ≤50 字兜底清洗（BYO-key）
