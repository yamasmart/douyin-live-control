...
- **快捷评论**：按节奏 / 条数自动发自配话术；可一键从中控台**以最新预设为准覆盖同步**快捷回复（同步前先刷新页面拿服务端最新值，避免继续发到百应后台已改 / 已删的旧话术）。
- **AI 扩写**：给任一条快捷评论一键生成、或在草稿上扩写一条 ≤50 字、突出卖点和福利、引导下单的口播式短评论（据本场商品名）。走 OpenAI 兼容接口，密钥由用户在「设置」里**自填、仅存本机**（BYO-key），不内置、不上传。每个用到大模型的功能都是「设置 → AI 任务」里的独立任务：可单独换模型、改提示词，并写明用途；没单独配的任务用默认模型和内置提示词。
...
- **运行日志**：每账号记录启动/讲解/评论/福袋/下播/异常（带时间），随时核对软件实际动作。
- **配置备份**：「设置」里一键把全部账号的讲解/评论/福袋配置导出成 .json，换电脑或重装后导入即可恢复。导入按账号合并（同一账号覆盖、新账号新增、其他不动），并校验每个字段。登录态不在备份里，导入后重新扫码；AI 密钥可选是否带上。
- **自动更新**：启动自动检查新版本——Windows 自动下载并提示重启安装；macOS 提示并引导前往下载页（未签名暂不支持静默更新）。
...
src/main/
  ...
  llm.ts             AI 任务登记表（AI_TASKS：名称/用途/内置提示词）+ OpenAI 兼容调用 + 扩写兜底清洗（BYO-key）
  backup.ts          配置备份：导出/导入 .json（剥离登录态、导入逐字段校验 + 数值兜底）
